...
## utilities
brew install the_silver_searcher tlrc curl wget jq p7zip wtf
brew install htop calc cointop gnupg watchman git-delta diff-so-fancy fd gh ripgrep neofetch netcat bottom procs zip mole
## server shizzle
...
### C / C++
brew install global cmake cmake-docs boost nasm vcpkg folly llvm autoconf automake ccache
### Wasm
